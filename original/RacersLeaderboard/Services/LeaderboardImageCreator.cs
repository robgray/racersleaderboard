...
{
	public class LeaderboardImageCreator
	{
		private readonly HttpServerUtilityBase _serverUtility;

		public LeaderboardImageCreator(HttpServerUtilityBase serverUtility)
		{
	    ...
	    public Image CreateTimeTrialLeaderboad(List<TimeTrialLeaderboard.TimeTrialItem> timeTrials)
	    {
	        const float COL_RANK = 5;
	        const float COL_NAME = 40;
	        const float COL_DIVISION = 250;
	        ...
	        using (var g = Graphics.FromImage((Image)board))
	        {
	            ...
	            for (var i = 0; i < timeTrials.Count(); i++)
	            {
	                ...
	                g.DrawString((i + 1).ToString(), font, Brushes.Black, COL_RANK, y);
	                g.DrawString(_serverUtility.UrlDecode(driver.DisplayName), font, Brushes.Black, COL_NAME, y);
