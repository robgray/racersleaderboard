...
namespace RacersLeaderboard.Services
{
	public class iRacingScraperService
	{
		public CookieCollection LoginAndGetCookies()
		{
			...
			//string formParams = $"username={Uri.EscapeDataString(username)}&password={Uri.EscapeDataString(password)}&utcoffset=-600&todaysdate=";
			string formParams = $"username=rob.ragged@gmail.com&password=M4yd4y94&utcoffset=-600&todaysdate=";
		...
		//public DriverStats GetDriverStatsFromProfile(CookieCollection cookies, int driverId, string csvFilename)
		//{
		//    var url = $"http://members.iracing.com/membersite/member/CareerStats.do?custid={driverId}";

		//    const string startField = "buf = '{\"favTrack\":";
		...
		//       {
		//           memberPage = sr.ReadToEnd();
		//       }

		//    var startIndex = memberPage.IndexOf(startField);
		//    var endIndex = memberPage.IndexOf("}';", startIndex);
		//    var length = endIndex - startIndex;

		//    var json = memberPage.Substring(startIndex, length).Remove(0, 7);
